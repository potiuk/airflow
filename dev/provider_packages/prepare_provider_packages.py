...
def make_sure_remote_apache_exists_and_fetch():
    ...
    try:
        subprocess.check_call(
            ["git", "remote", "get-url", "apache-https-for-providers"],
...
def get_git_log_command(from_commit: Optional[str], to_commit: Optional[str] = "HEAD") -> List[str]:
    """
...
def update_generated_files_for_regular_package(
    ...
    if update_release_notes:
        ...
        if not proceed:
            print()
            print(
                f"[yellow]Provider: {provider_package_id} - not proceeding with documentation generation[/]"
            )
            print()
            sys.exit(64)
...
def list_providers_packages(args: Any):
...
def generate_setup_files(args: Any):
    ...
    with with_group(f"Generate setup files for {provider_package_id}"):
        ...
        print(f"[green]Generated setup files for {provider_package_id}[/]")
...
def build_provider_packages(args: Any):
...
def verify_provider_classes(args: Any):
