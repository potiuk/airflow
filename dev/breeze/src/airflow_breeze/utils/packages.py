...
def refresh_provider_metadata_from_yaml_file(provider_yaml_path: Path):
    ...
    if provider_yaml_path.is_relative_to(AIRFLOW_PROVIDERS_SRC_DIR):
...
def refresh_provider_metadata_with_provider_id(provider_id: str):
    provider_yaml_path = get_old_base_package_path(provider_id) / "provider.yaml"
...
def get_old_base_package_path(provider_id: str) -> Path:
...
def load_pyproject_toml(pyproject_toml: Path) -> dict[str, Any]:
    try:
        import tomllib
    except ImportError:
        import tomli as tomllib
    return tomllib.loads(pyproject_toml.read_text())
...
def get_provider_details(provider_id: str) -> ProviderPackageDetails:
    ...
    if is_new_structure:
        ...
        changelog_path = provider_yaml_path.parent / "CHANGELOG.rst"
        documentation_provider_package_path = get_new_documentation_package_path(provider_id)
        base_provider_package_path = provider_yaml_path.parent
    else:
        dependencies = provider_info["dependencies"]
        changelog_path = get_old_base_package_path(provider_id) / "CHANGELOG.rst"
        documentation_provider_package_path = get_old_documentation_package_path(provider_id)
        base_provider_package_path = get_old_base_package_path(provider_id)
