...
class TriageTUI:
    ...
    def get_available_actions(entry: PRListEntry) -> list[TUIAction]:
        ...
        if cat == PRCategory.WORKFLOW_APPROVAL:
            actions = [TUIAction.ACTION_RERUN]
            if not pr.is_draft:
                actions.append(TUIAction.ACTION_DRAFT)
            actions.extend([TUIAction.ACTION_COMMENT, TUIAction.ACTION_CLOSE])
            return actions
    ...
    def _build_footer(self) -> Panel:
        ...
        if entry is not None:
            ...
            if available and not entry.action_taken:
        ...
        lines = [nav, actions]
        if direct_line:
            lines.append(f"[dim]Quick actions:[/] {direct_line}")
        footer_text = "\n".join(lines)
        return Panel(
    ...
    def render(self) -> None:
        ...
        footer_height = 5  # up to three lines of keys + border
