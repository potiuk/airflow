...
_CHECK_CATEGORIES: list[tuple[str, list[str], str, str]] = [
    ...
    (
        ...
        ["ruff"],
        "Run `prek run ruff --ref-from main` and `prek run ruff-format --ref-from main` to fix.",
...
def _categorize_check(check_name: str) -> tuple[str, str, str] | None:
    """Match a failing check name to a category. Returns (category, fix, doc_url) or None."""
