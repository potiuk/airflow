    ...
    for providers in provider_folders:
        args.extend(
            [
                ...
                f"--ignore=tests/providers/system/{providers}",
                f"--ignore=tests/providers/integration/{providers}",
            ]
        )
    return args
    ...
    if test_group == GroupOfTests.PROVIDERS:
        ...
        if test_type.startswith(PROVIDERS_LIST_PREFIX):
            ...
            for provider in provider_list:
                # TODO(potiuk): remove me when all providers are migrated
                provider_path = (
                    (AIRFLOW_SOURCES_ROOT / "tests" / "providers")
                    .joinpath(provider.replace(".", "/"))
                    .relative_to(AIRFLOW_SOURCES_ROOT)
                )
