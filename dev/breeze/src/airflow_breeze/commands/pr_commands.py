...
_CHECK_FAILURE_GRACE_PERIOD_HOURS = 4
...
def _are_failures_recent(token: str, github_repository: str, head_sha: str) -> bool:
    """Check if all completed failed workflow runs for a commit finished within the grace period.

    Returns True if ALL failures are recent (within _CHECK_FAILURE_GRACE_PERIOD_HOURS),
    ...
    for run in failed_runs:
        ...
        try:
            ...
            if age_hours > _CHECK_FAILURE_GRACE_PERIOD_HOURS:
...
def _fetch_unresolved_comments_batch(token: str, github_repository: str, prs: list[PRData]) -> None:
    ...
    for chunk_start in range(0, len(prs), _REVIEW_THREADS_BATCH_SIZE):
        ...
        for pr in chunk:
            alias = f"pr{pr.number}"
            # Fetch all comments in each thread (up to 20) so we can find the author's last reply
            pr_fields.append(
...
def _prompt_review_comment_action(comment_summary: str, *, with_open: bool = False) -> str:
    """Prompt user for action on a single review comment: [s]ubmit, [e]dit, [o]pen, s[k]ip.

    Returns 's' for submit, 'e' for edit, 'o' for open, 'k' for skip, 'q' for quit.
    """
    from airflow_breeze.utils.confirm import _read_char

    get_console().print(f"\n  {comment_summary}")
    if with_open:
        get_console().print("  \\[S]ubmit / \\[e]dit / \\[o]pen / s\\[k]ip / \\[q]uit: ", end="")
    else:
        get_console().print("  \\[S]ubmit / \\[e]dit / s\\[k]ip / \\[q]uit: ", end="")
    try:
        ch = _read_char()
    except (KeyboardInterrupt, EOFError):
        get_console().print()
    ...
    if len(ch) > 1:
        get_console().print()
        return "s"  # default: submit
    get_console().print(ch)
    valid = ("s", "e", "o", "k", "q") if with_open else ("s", "e", "k", "q")
    return ch.lower() if ch.lower() in valid else "s"
...
def _interactive_review_comments(
    ...
    *,
    ...
    """Walk through LLM-proposed review comments one-by-one and submit/skip/edit each.
    ...
    for i, comment in enumerate(review.comments, 1):
        ...
        if _post_review_comment(
            ...
            console.print(f"  [success]Comment submitted on {comment.path}:{comment.line}.[/]")
            submitted_count += 1
            ...
            console.print(f"  [error]Failed to submit comment on {comment.path}:{comment.line}.[/]")

    if not ctx.stats.quit_early:
        parts = [f"{submitted_count} submitted", f"{skipped_count} skipped"]
        if duplicate_count:
    ...
    *,
    ...
    try:
        ...
        for pr in accepted_prs:
            ...
            # Grace period: don't flag CI failures that are less than 24 hours old
            if (
                ci_assessment
                and pr.head_sha
                and _are_failures_recent(ctx.token, ctx.github_repository, pr.head_sha)
            ):
                ci_assessment = None
        ...
        if recent_failures_skipped:
            ...
            console.print(
                f"\n[info]Skipped {len(recent_failures_skipped)} {pr_word} with CI failures "
                f"less than {_CHECK_FAILURE_GRACE_PERIOD_HOURS}h old "
                f"(giving authors time to address):[/]"
            )
...
def _display_triage_summary(
    ...
    *,
    ...
    summary_table.add_row(
        f"Recent CI failures <{_CHECK_FAILURE_GRACE_PERIOD_HOURS}h (skipped)",
...
def auto_triage(
    ...
    if run_api:
        for pr in candidate_prs:
            ...
            # Grace period: don't flag CI failures that are less than 24 hours old
            if ci_assessment and pr.head_sha and _are_failures_recent(token, github_repository, pr.head_sha):
                ci_assessment = None
                # If CI is the only issue, skip this PR entirely
                conflict_check = assess_pr_conflicts(pr.number, pr.mergeable, pr.base_ref, pr.commits_behind)
                comments_check = assess_pr_unresolved_comments(
    ...
    if recent_failures_skipped:
        get_console().print(
            ...
            f"with CI failures less than {_CHECK_FAILURE_GRACE_PERIOD_HOURS}h old "
            f"(giving authors time to address):[/]"
        )
        for pr in recent_failures_skipped:
            get_console().print(f"  [dim]{_pr_link(pr)} — {pr.title[:70]}[/]")
        ...
        if run_api:
            for pr in candidate_prs:
                ...
                if (
                    ci_assessment
                    and pr.head_sha
                    and _are_failures_recent(token, github_repository, pr.head_sha)
                ):
                    ci_assessment = None
                    # If CI is the only issue, skip this PR entirely
                    ...
                    if not conflict_check and not comments_check:
                        get_console().print(
                            ...
                            f"{_CHECK_FAILURE_GRACE_PERIOD_HOURS}h old[/]"
                        )
                        continue
