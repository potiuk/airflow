...
Pin xcom-sidecar default image to ``alpine:3.23``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
to ``alpine:latest``) to a pinned ``alpine:3.23``. The pin makes kubelet's default
``imagePullPolicy`` ``IfNotPresent`` instead of ``Always``, so a node with the
image cached does not re-pull on every task — protecting deployments and CI from
