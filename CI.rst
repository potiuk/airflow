...
|                                                         Verbosity variables                                                        |
+-----------------------------------------+-------------+-------------+------------+-------------------------------------------------+
| ``NO_TERMINAL_OUTPUT_FROM_SCRIPTS``     |    false    |     false   |   false    | Suppresses output to terminal from running      |
|                                         |     (x)     |      (x)    |    (x)     | scripts. Ouput will be printed only in case     |
|                                         |             |             |            | of commands failing. Set to "true" for static   |
|                                         |             |             |            | checks executed within pre-commit framework.    |
|                                         |             |             |            |                                                 |
|                                         |             |             |            | (x) set to true in pre-commits                  |
+-----------------------------------------+-------------+-------------+------------+-------------------------------------------------+
...
|                                         |             |             |     (x)    | made to rebuild the image with latest           |
...
you interact with "apache/airflow" DockerHub repository, but you can change that to your own repository
by setting those environment variables:
...
"Build Images" workflow. It's possible to disable this feature and go back to the previous behaviour via
...
enable some integrations. Therefore it is easiest to use `Breeze <BREEZE.rs>`_ for that. For example if
