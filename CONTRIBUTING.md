...
All details about using and running Airflow Breeze can be found in [BREEZE.md](BREEZE.md)
...
Should you add or upgrade a npm package, which involves changing [package.json](airflow/www/package.json)
you'll need to re-run `npm install` and push the newly generated
[package-lock.json](airflow/www/package-lock.json) file so we get the reproducible builds.
In the CI  environment we run `npm ci` which install the exact version od dependencies stored in the
[package-lock.json](airflow/www/package-lock.json) file
