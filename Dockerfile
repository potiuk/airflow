...
# Move node_modules away so that we do not have to copy it
RUN mv ${AIRFLOW_SOURCES}/airflow/www/node_modules ${AIRFLOW_SOURCES}/node_modules
...
RUN pip install --user --no-use-pep517 \
        "https://github.com/${AIRFLOW_REPO}/archive/${AIRFLOW_BRANCH}.tar.gz#egg=apache-airflow[all]" \
...
RUN pip install --user --no-use-pep517 -e ".[${AIRFLOW_EXTRAS}]" \
...
COPY --from=airflow-www ${AIRFLOW_SOURCES}/airflow/www ${AIRFLOW_SOURCES}/airflow/www

# Copy Airflow www packages
COPY --from=airflow-www ${AIRFLOW_SOURCES}/node_modules ${AIRFLOW_SOURCES}/airflow/www/node_modules
...
# Copy Airflow www packages
COPY --chown=airflow:airflow --from=airflow-www ${AIRFLOW_SOURCES}/airflow/www ${HOME}/.local/airflow/www
