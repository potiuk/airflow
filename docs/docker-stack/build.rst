...
* You can use regular ``pip install`` commands (and as of Dockerfile coming in Airflow 2.9 also
  ...
  accidentally upgrading or downgrading the version of Apache Airflow. Depends on the scenario you might
...
used to install airflow. This allows you to use version of packages that you know were tested with the
given version of Airflow, so you can use it if you do not want to use potentially newer versions
...
experimental feature as ``uv`` is a very fast but also very new tool in Python ecosystem.
...
This is experimental feature as ``uv`` is a very fast but also very new tool in Python ecosystem.
