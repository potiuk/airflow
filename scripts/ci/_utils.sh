...
Docker pulling ${PYTHON_BASE_IMAGE}.
        ...
        if [[ ${CACHED_AIRFLOW_CI_IMAGE:=} != "" ]]; then
            IMAGE_TO_PULL="${CACHED_AIRFLOW_CI_IMAGE}"
        else
            IMAGE_TO_PULL="${AIRFLOW_CI_IMAGE}"
        fi
        IMAGE_HASH=$(docker images -q "${IMAGE_TO_PULL}" 2> /dev/null)
        if [[ "${IMAGE_HASH}" == "" ]]; then
            PULL_IMAGE="true"
        fi
...
" > "${DETECTED_TERMINAL}"
            ...
            verbose_docker pull "${IMAGE_TO_PULL}" | tee -a "${OUTPUT_LOG}" || true
            if [[ ${CACHED_AIRFLOW_CI_IMAGE} != "" ]]; then
                verbose_docker tag "${CACHED_AIRFLOW_CI_IMAGE}" "${AIRFLOW_CI_IMAGE}"
            fi
            echo
        fi
    fi
...
function pull_prod_images_if_needed() {
    ...
    if [[ "${DOCKER_CACHE}" == "pulled" ]]; then
        ...
        if [[ ${CACHED_AIRFLOW_PROD_IMAGE:=} != "" ]]; then
            IMAGE_TO_PULL="${CACHED_AIRFLOW_PROD_IMAGE}"
            IMAGE_TO_PULL_BUILD="${CACHED_AIRFLOW_PROD_BUILD_IMAGE}"
        else
            IMAGE_TO_PULL="${AIRFLOW_PROD_IMAGE}"
            IMAGE_TO_PULL_BUILD="${AIRFLOW_PROD_BUILD_IMAGE}"
        fi
        BUILD_IMAGE_HASH=$(docker images -q "${IMAGE_TO_PULL_BUILD}" 2> /dev/null)
        if [[ "${BUILD_IMAGE_HASH}" == "" ]]; then
            PULL_BUILD_IMAGE="true"
        fi
        if [[ "${PULL_BUILD_IMAGE}" == "true" ]]; then
            echo
            echo "Pulling the image ${IMAGE_TO_PULL_BUILD}"
            echo
            verbose_docker pull "${IMAGE_TO_PULL_BUILD}" | tee -a "${OUTPUT_LOG}" || true
            echo
            if [[ ${CACHED_AIRFLOW_PROD_BUILD_IMAGE} != "" ]]; then
                verbose_docker tag "${CACHED_AIRFLOW_PROD_BUILD_IMAGE}" "${AIRFLOW_PROD_BUILD_IMAGE}"
            fi
        fi
        local PULL_IMAGE=${FORCE_PULL_IMAGES}
        local IMAGE_HASH
        ...
        if [[ "${PULL_IMAGE}" == "true" ]]; then
            ...
            verbose_docker pull "${IMAGE_TO_PULL}" | tee -a "${OUTPUT_LOG}" || true
            if [[ ${CACHED_AIRFLOW_PROD_IMAGE} != "" ]]; then
                verbose_docker tag "${CACHED_AIRFLOW_PROD_IMAGE}" "${AIRFLOW_PROD_IMAGE}"
            fi
            echo
        fi
    fi
...
function prepare_ci_build() {
    ...
    if [[ ${CACHE_REGISTRY=""} != "" && ${DISABLE_CACHE_REGISTRY} != "true" ]]; then
