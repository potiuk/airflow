...
#
# Adds extra container docker flags - for example mounted volumes or extra environment variables
# Input: AIRFLOW_MOUNT_HOST_VOLUMES_FOR_STATIC_CHECKS (whether to mount volumes)
# Output: AIRFLOW_MOUNT_HOST_VOLUMES_FOR_STATIC_CHECKS, AIRFLOW_CONTAINER_EXTRA_DOCKER_FLAGS (array)
#
...
function _confirm_image_rebuild() {
    ...
    if [[ ${CI:="false"} == "true" ]]; then
        ...
        "${AIRFLOW_SOURCES}/confirm" "Rebuild image ${THE_IMAGE_TYPE}" </dev/tty >/dev/tty
        RES=$?
    elif [[ -t 0 ]]; then
