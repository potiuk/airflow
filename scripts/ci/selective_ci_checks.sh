...
if [[ ${PR_LABELS=} == *"okay to test"* ]]; then
    echo
    echo "Found the right PR labels in '${PR_LABELS=}': 'okay to test''"
    echo
    OKAY_TO_TEST="true"
else
    echo
    echo "Did not find the right PR labels in '${PR_LABELS=}': 'okay to test'"
    echo
    OKAY_TO_TEST="false"
fi

function output_all_basic_variables() {
    if [[ ${OKAY_TO_TEST} == "true" ]]; then
...
if (($# < 1)); then
    ...
    echo
    # override OKAY_TO_TEST in master/scheduled run
    OKAY_TO_TEST="true"
    readonly OKAY_TO_TEST
