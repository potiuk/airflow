...
# With silent echo we only print to output log
function silent_echo() {
    echo "${@}" >"${OUTPUT_LOG}"
}

# With verbose echo we print to output log and to stdout
function verbose_echo() {
    echo "${@}" | tee -a "${OUTPUT_LOG}"
}

function initialize_verbosity_variables() {
    # whether the output from script should be printed at all
    export NO_TERMINAL_OUTPUT_FROM_SCRIPTS=${NO_TERMINAL_OUTPUT_FROM_SCRIPTS:="false"}

    if [[ ${NO_TERMINAL_OUTPUT_FROM_SCRIPTS} == "true" ]]; then
        alias echo=silent_echo
    else
        alias echo=verbose_echo
    fi
...
function initialize_common_environment() {
    ...
    initialize_verbosity_variables
