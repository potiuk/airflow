...
function push_ci_images_to_dockerhub() {
    ...
    if [[ -n ${DEFAULT_CI_IMAGE:=""} ]]; then
        ...
        verbose_docker push "${DEFAULT_CI_IMAGE}"
    fi
}
...
function push_ci_images_to_github() {
    ...
    verbose_docker push "${AIRFLOW_CI_TAGGED_IMAGE}"
    if [[ ${GITHUB_SHA:=} != "" ]]; then
        ...
        verbose_docker push "${AIRFLOW_CI_SHA_IMAGE}"
    fi
    PYTHON_TAG_SUFFIX=""
    if [[ ${GITHUB_REGISTRY_PUSH_IMAGE_TAG} != "latest" ]]; then
        PYTHON_TAG_SUFFIX="-${GITHUB_REGISTRY_PUSH_IMAGE_TAG}"
    fi
    verbose_docker tag "${PYTHON_BASE_IMAGE}" "${GITHUB_REGISTRY_PYTHON_BASE_IMAGE}${PYTHON_TAG_SUFFIX}"
    verbose_docker push "${GITHUB_REGISTRY_PYTHON_BASE_IMAGE}${PYTHON_TAG_SUFFIX}"
}
...
function push_prod_images_to_dockerhub () {
    ...
    if [[ -n ${DEFAULT_PROD_IMAGE:=""} ]]; then
        verbose_docker push "${DEFAULT_PROD_IMAGE}"
    fi
    # Prod build image
...
function push_prod_images_to_github () {
    ...
    verbose_docker push "${GITHUB_REGISTRY_AIRFLOW_PROD_IMAGE}:${GITHUB_REGISTRY_PUSH_IMAGE_TAG}"
    if [[ ${COMMIT_SHA:=} != "" ]]; then
        # Also push image to GitHub registry with commit SHA
        ...
        verbose_docker push "${AIRFLOW_PROD_SHA_IMAGE}"
    fi
    ...
    verbose_docker push "${AIRFLOW_PROD_BUILD_TAGGED_IMAGE}"
}
