...
class TestSuperviseCallbackExchangesTokenBeforeExecuting:
    """The callback token is exchanged through the Execution API *before* the callback is run.

    This is the boundary that stops an unauthenticated message on the broker from reaching callback
    execution: the exchange redeems the single-use token, and nothing runs until it does.
    """
    ...
    def test_exchange_runs_before_the_subprocess_starts(self):
        order = []
        client = Mock()
        client.callbacks.run.side_effect = lambda *a, **k: order.append("exchange")
        ...
        assert order == ["exchange", "start"], "the token must be exchanged before the callback starts"

    def test_a_rejected_exchange_prevents_the_subprocess_from_starting(self):
        """If the server refuses the exchange (forged/mismatched/replayed token), nothing executes."""
        client = Mock()
